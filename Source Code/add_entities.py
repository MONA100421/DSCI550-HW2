...
# Base URL for images (ensure your HTTP server is running and serving the images)
...
def main():
    ...
    # Get start and end indices from command line arguments, defaulting to 0-100 if not provided.
    start = int(sys.argv[1]) if len(sys.argv) > 1 else 0
    end = int(sys.argv[2]) if len(sys.argv) > 2 else 100

    for i in range(start, min(end, len(df))):
        # Only process if this row's detected_objects field is empty.
        if pd.notna(df.loc[i, "detected_objects"]) and df.loc[i, "detected_objects"] != "":
            continue
        
        rel_path = df.loc[i, "ai_image_path"]  # e.g., "images/haunted_image_XXXX.png"
